...
module.exports = async function handler(req, res) {
  ...
  const configuredEmail = String(process.env.DEMO_USER_EMAIL || "")
    .trim()
    .toLowerCase();
  ...
  if (
    ...
    !safeEqual(accessCode, configuredCode)
  ) {
    return res.status(401).json({ error: "Invalid email or access code" });
  }

  // Group-specific namespace so the P/Q Consent Group gets its own global profile.
  const groupNamespace = String(
    process.env.CONSENT_GROUP_NAMESPACE || "",
  ).trim();
  const scopedIdentifier = groupNamespace
    ? `${groupNamespace}:${email}`
    : email;
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Could not create authentication token" });
  }
};
